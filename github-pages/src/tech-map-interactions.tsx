...
const details: Record<string, Detail> = {
  'node-snowflake': { title: 'Snowflake', summary: 'Analytics-ready transformations designed to move source data toward reliable reporting models.', built: 'Worked with staged-to-mart transformations that make the path from raw source data to analytical consumption explicit.', patterns: ['Layered staging and mart models', 'Incremental processing where appropriate', 'SQL performance investigation'], outcome: 'Clearer, more dependable data sets for downstream analysis.' },
  'node-dbt': { title: 'dbt', summary: 'Transformation work shaped around readable, maintainable model layers.', built: 'Used dbt-style model layering to organize transformations from preparation through curated analytical tables.', patterns: ['Modular SQL transformations', 'Staging-to-mart flow', 'Incremental model patterns'], outcome: 'A more legible transformation workflow that supports analytical use.' },
  'node-databricks': { title: 'Databricks', summary: 'Distributed processing for telemetry-style data pipelines.', built: 'Worked on Databricks and PySpark pipelines for transforming IoT-style telemetry into useful downstream data sets.', patterns: ['PySpark transformations', 'Telemetry event processing', 'Notebook-driven pipeline work'], outcome: 'High-volume events shaped into data that is easier to use downstream.' },
  'node-azure': { title: 'Azure IaC', summary: 'Cloud foundations approached with repeatability and operational clarity in mind.', built: 'Applied infrastructure-as-code practices in Azure-oriented cloud data work to make environments and delivery more reproducible.', patterns: ['Infrastructure as code', 'Cloud data platform foundations', 'Environment-aware delivery'], outcome: 'More consistent foundations for data platform work.' },
  'node-sql': { title: 'SQL', summary: 'The connective language across backend foundations and analytical modeling.', built: 'Used SQL across relational data work, transformation modeling, performance troubleshooting, and application-facing data access.', patterns: ['Relational modeling', 'Analytical transformations', 'Query troubleshooting'], outcome: 'Queries shaped for both practical application needs and analytical clarity.' },
  'node-api': { title: 'APIs / Kafka', summary: 'Backend services built around clear data contracts, access control, and asynchronous movement.', built: 'Built FastAPI and MongoDB services alongside Kafka-backed caching and authorization controls; also worked with PostgreSQL, GraphQL, and RabbitMQ applications.', patterns: ['Service-oriented API design', 'Kafka-backed caching', 'Authorization and messaging'], outcome: 'Software systems with clearer boundaries around how data is accessed and moved.' },
};
...
export function TechMapInteractions() {
  ...
  return <div className="github-modal-backdrop" role="presentation" onMouseDown={() => setActive(null)}>
    <section className="github-tech-dialog" role="dialog" aria-modal="true" aria-labelledby="technology-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="dialog-grid"><section><h3>What he built</h3><p>{active.built}</p></section><section><h3>Work patterns</h3><ul>{active.patterns.map((pattern) => <li key={pattern}>{pattern}</li>)}</ul></section></div>
